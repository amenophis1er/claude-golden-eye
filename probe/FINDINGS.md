...
- **V1 consequence (PM enforcement):** a `PreToolUse` deny on Write/Edit that fires only when the
  payload *lacks* `agent_id` = main-session-only blocking. Feasible, now proven at the payload
  level. Remaining M2 check: one live smoke test that a deny decision actually blocks + guides.
...
`prompt_id` + time proximity (or by diffing the `subagents/` directory). Acceptable for V0;
flagged for M1.
...
6. `UserPromptSubmit` carries the full prompt text — the V1 re-anchor has everything it needs.
...
8. **Denied tool calls are visible in the event stream** (observed live during M1): a denied or
   erroring call fires `PreToolUse` but never `PostToolUse` — a Pre-without-Post pair is a free
   denial/blocked-call signal for the Observer's timeline. (The sandboxed M1 run showed 8 such
...
9. **Slash-command namespace collision (M2 smoke test):** unknown `/commands` are rejected
...
10. **The `PreToolUse` deny JSON contract is empirically confirmed** (M2 smoke test):
...
11. **MCP plugin wiring confirmed live** (M3): a hand-rolled stdio JSON-RPC server
...
- V0 Observer: the full agent tree (spawn reason, live per-child tool calls, final report,
  durations) is derivable from hooks **alone**; transcript tailing is the fidelity booster, not
  a crutch. Hybrid path confirmed.
- V1 PM mode: enforcement via `agent_id` discrimination is viable; re-anchor via
  `UserPromptSubmit` is viable. Both risks from §8 are retired; remaining open item is the live
  deny smoke test (M2).
